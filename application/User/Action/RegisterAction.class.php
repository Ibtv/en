	...
	function doregister(){
    	...
    	if($result || $uc_checkemail<0 || $uc_checkusername<0){
    		...
    		if($uc_register){
                ...
                $users_model_ar = M("ibtv_ar.Users","i_");
                $users_model_es = M("ibtv_es.Users","i_");
                $users_model_fr = M("ibtv_fr.Users","i_");
                $users_model_pt = M("ibtv_pt.Users","i_");
                $users_model_ru = M("ibtv_ru.Users","i_");
                $users_model_de = M("ibtv_de.Users","i_");
                $users_model_kr = M("ibtv_kr.Users","i_");
                $users_model_tw = M("ibtv_tw.Users","i_");
                $users_model_cmf = M("ibtv_cmf.Users","i_");
    			$rst = $users_model->add($data);
                $rst_ar = $users_model_ar -> add($data);
                $rst_es = $users_model_es -> add($data);
                $rst_fr = $users_model_fr -> add($data);
                $rst_pt = $users_model_pt -> add($data);
                $rst_ru = $users_model_ru -> add($data);
                $rst_de = $users_model_de -> add($data);
                $rst_kr = $users_model_kr -> add($data);
                $rst_tw = $users_model_tw -> add($data);
                $rst_cmf = $users_model_cmf -> add($data);
    			if($rst && $rst_ar && $rst_es && $rst_fr && $rst_pt && $rst_ru && $rst_de && $rst_kr && $rst_tw && $rst_cmf){
    				//登入成功页面跳转
    				$data['id']=$rst;
    				$_SESSION['user']=$data;
